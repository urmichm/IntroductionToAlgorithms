...
bool topological_sort(Node* &node, set<int>& path, vector<int> &sorted_order);
...
bool topological_sort(Node* &node, set<int>& path, vector<int> &sorted_order)
{
	...
	for (Node* next : node->neighbors)
	{
		if (next->visited == false) {
			result = topological_sort(next, path, sorted_order);
		}
		else if (path.count(next->val) > 0) 
		{
			result = false;
		}
		else 
	...
	sorted_order.push_back(node->val);
	path.erase(node->val);
	return result;
}
...
vector<int> findOrder(int numCourses, vector<vector<int>>& prerequisites) 
{
	int res_size = 0, temp = 0, i = 0;
	vector<int> res;
	set<int> path;
	vector<Node> adjList;
	bool isOk = true;
	/// There are a total of n courses labelled from 0 to n - 1.
	...
	for (i = 0; isOk && (i < numCourses); i++) {
		if (adjList[i].visited == false) {
			Node* n = &adjList[i];
			isOk = topological_sort(n, path, res);
		}
	}
...
void run(void)
{
	...
	// 3.
	{
		...
		vector<int> res = findOrder(1, prerequisites);
		for (int a : res)
			cout << a << " ";
		cout << "." << endl;
	}
